...
def evaluate_model(
    ...
    ######################################## 
    # save predictions and sensitive attributes
    test_pred_df = X_test[groups]
    test_pred_df.loc[:,'y_true'] =y_test
    test_pred_df.loc[:,'y_pred'] = est.predict(X).reshape(-1,1)
    test_pred_df.loc[:,'y_pred_proba'] = est.predict_proba(X_test)[:,1]
    ...
    # predictions
    test_pred_df.to_csv(save_file + '.csv')
    # stats
    if hasattr(est, 'stats_'):
        est.stats_.to_csv(save_file + '.stats')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rho', action='store', default=0.001, type=float, 
                        help='Min subpop prevalence (for metrics)')
    parser.add_argument('-ohc', action='store', default=None, type = str,
    ...
    parser.add_argument("-text", action='store',type=str, default='chiefcomplaint', 
    ...
    if args.ohc is None:
        ohc = 'ohc' if 'lr' in args.ml else 'label_encoding'
    else:
        ohc = args.ohc 
