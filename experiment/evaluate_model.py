...
from pmcboost.pmc.auditor import Auditor
from pmcboost.pmc.metrics import (differential_calibration, multicalibration_score,
...
def evaluate_model(dataset,
    ...
    features, labels = read_file(dataset,
                                 one_hot_encode=one_hot_encoded,
                                 label='y',
    ...
    for fold, target, X in zip(['train', 'test'], [y_train, y_test],
        ...
        for score, scorer in [('roc_auc', roc_auc_score),
            ...
            print(score + '_' + fold, f'{scorer(target, y_pred_proba):.3f}')
        for score, scorer in [('accuracy', accuracy_score)]:
            results[score + '_' + fold] = scorer(target, y_pred)
            print(score + '_' + fold, f'{scorer(target, y_pred):.3f}')
        ...
        print('MC_loss_' + fold, f"{results['MC_loss_' + fold]:.3f}")
        # PMC
        ...
        print('PMC_loss_' + fold, f"{results['PMC_loss_' + fold]:.3f}")
        # differential calibration loss
        results['DC_loss_' + fold] = differential_calibration(estimator=est,
                                                              y_true=target,
                                                              X=X,
                                                              groups=groups,
                                                              n_bins=n_bins,
                                                              ...
                                                              gamma=gamma,
                                                              rho=rho)
        print('DC_loss_' + fold, f"{results['DC_loss_' + fold]:.3f}")
    ...
    save_file = os.path.join(
        results_path, '_'.join([
            f'{n}' for n in [
                ...
                one_hot_encoded,
                random_state,
                os.environ['LSB_JOBID'] if 'LSB_JOBID' in
                os.environ.keys() else '',
...
if __name__ == '__main__':
    ...
    parser.add_argument('-emb_path',
                        action='store',
                        dest='EMB',
                        default='preprocessing/BCH_embedding.npy',
                        type=str,
    ...
    parser.add_argument("-text",
                        ...
                        help='Specify text features with comma seperated')
    args = parser.parse_args()
    groups = args.groups.split(',')
    # import algorithm
    print('import from', 'ml.' + args.ml)
    algorithm = importlib.__import__('ml.' + args.ml, globals(), locals(),
                                     ['*'])
    ...
    evaluate_model(dataset=args.file,
